...
if a == 0 and b !=0 and c !=0: # Verifica a condição de uma equação a.x^2 + c = 0
    ...
    if c > 0: #correção do programa, foi necessário incluir um if para realizar o tratamento do valor c > 0
        print("Quando o coeficiente c for positivo, não será possível encontrar soluções reais!")
    else:
        x1 = sqrt (-c/a)
        x2 = -sqrt (-c/a)
        print("As raizes são x' = {0:2.3f} e x'' = {1:2.3f}".format(x1,x2))
    ...
    if delta > 0:
        ...
        print("Não possui raiz real, a parábola não intercepta o eixo x.")